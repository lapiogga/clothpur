...
- **처리 로직**:
  ...
  3. 완제품: 판매소 재고 확인 (BR-015) - store_id는 항상 필수 (판매소 선택 선행)
  4. 직접배송 시 delivery_zone_id 유효성 검증 (해당 판매소 소속 + 활성 상태)
  5. 트랜잭션:
...
| 버전 | 날짜 | 변경 내용 | 작성자 |
|------|------|----------|--------|
| v1.0 | 2026-03-04 | 최초 작성 | Planner |
| v1.1 | 2026-03-05 | F-M6-002 처리 로직 수정 (판매소 선택 선행 + 직접배송 배송지 유효성 검증 단계 추가) | developer |
