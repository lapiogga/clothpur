...
| 역할 | 영문 코드 | 설명 | 주요 업무 |
|------|-----------|------|-----------|
| 군수담당자 | `admin` | 시스템 전체 관리자 | 사용자/포인트/카테고리/품목/판매소/체척업체/체척권/메뉴 관리, 체척권 정산처리 |
| 피복판매소 담당자 | `store` | 피복 판매 현장 운영자 | 오프라인 판매, 온라인 주문 배송, 재고/입고/반품, 통계 |
| 체척업체 담당자 | `tailor` | 맞춤피복 가공 업체 | 체척권 등록, 현황 조회, 정산 요청 |
| 일반사용자 | `user` | 장교, 부사관, 군무원 | 온라인 구매, 구매취소, 구매내역/배송 확인, 포인트/체척권 조회 |
...
| REQ-A-004 | 사용자 정보 수정 | 필수 | 이름, 계급, 소속, 진급일, 퇴직예정일, 퇴직예정일 변경 시 포인트 차감 트리거, 상태(활성/비활성) 수정. 계급 변경 시 진급일 필수 입력, 진급 포인트 추가 자동 트리거 |
...
| REQ-A-016 | 퇴직예정자 포인트 차감 | 필수 | 퇴직예정일 변경 시, 잔여기간(1/1~퇴직예정일) 일할계산하여 차감 |
| REQ-A-017 | 포인트 임의 변경 | 필수 | 사용자의 포인트 임의 추가 지급/차감, 변경일/사유 등록 |
...
| REQ-A-022 | 품목 CRUD | 필수 | 품목명, 대분류, 중분류, 소분류, 품목유형(완제품/맞춤피복), 단가, 이미지, 설명, 사용 여부 |
...
| REQ-S-004 | 판매 내역 조회 | 필수 | 기간/사용자명/품목유형 필터, 주문번호·사용자·품목유형·품목명·수량·포인트·판매일 표시 |
...
| 요구사항 ID | 요구사항 | 우선순위 | 상세 |
|------------|---------|---------|------|
| REQ-S-010 | 온라인 주문 목록 | 필수 | 상태별 탭(전체/대기/확인/배송중/완료/취소), 주문번호·사용자·품목유형·품목명·수량·포인트·배송방법·상태·주문일 표시 |
...
| 요구사항 ID | 요구사항 | 우선순위 | 상세 |
|------------|---------|---------|------|
| REQ-S-030 | 일별 판매현황 | 필수 | 기간 선택, 날짜별 판매건수·판매포인트 집계, 합계 행 |
| REQ-S-031 | 품목별 판매현황 | 필수 | 기간+분류 필터, 품목명·규격·판매수량·판매포인트 집계 |
| REQ-S-032 | 사용자별 판매현황 | 필수 | 기간+사용자검색 필터, 사용자명·계급·구매건수·구매포인트 집계 |
| REQ-S-033 | 대시보드 | 필수 | 온라인 주문 수량(대기/확인/배송중/완료), 오프라인 판매 건수/포인트 집계, 재고 부족 품목수, 배송지 등록 건수  |
...
| BR-017 | 품목 분류 | 동일한 대분류 내에서 중분류 선택, 동일한 중분류 내에서 소분류 선택 |
